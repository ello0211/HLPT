...
def main(
    ...
    save_file = f'experiment/00/{args.model}-{args.adapter}-{args.dataset}.json'
    create_dir('experiment/00/')

    dataset = load_data(args)
    # dataset = load_dataset("glue", 'cola') # 修改4
    batches = create_batch(dataset, args.batch_size) # 非常奇怪，不知道是干嘛用的
    ...
    for idx, batch in enumerate(batches):
        ...
        with open(save_file, 'w+') as f:
            ...
            json.dump(correct / current, f, indent=4)
        pbar.update(1)
    pbar.close()
    print('\n')
    print('test finished')
