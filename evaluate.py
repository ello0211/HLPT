...
def main(
    ...
    for instruction in [
        ...
        print()
    """
    save_file = f'experiment/test/{args.model}-{args.adapter}-{args.dataset}.json'
    create_dir('experiment/test/')

    dataset = load_data(args)
    tokenizer, model = load_model(args) # 这里有点类似于finetune.py的第一二三四步，对模型进行加载
    total = len(dataset)
    correct = 0
    miss = 0.001
    output_data = []
