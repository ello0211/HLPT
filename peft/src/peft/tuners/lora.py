...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, config, model):
        ...
        self.model = model
        self._find_and_replace()  # 感觉就是到这里new好像没啥用啊，再往后看看
    ...
    def _find_and_replace(self):
        ...
        key_list = [key for key, _ in self.model.named_modules()] # 这里key_list会涵盖每一层tansformer内的所有的层
        for key in key_list:
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        nn.Linear.__init__(self, in_features, out_features, **kwargs) # 这里跳到reset_parameters处，对weight进行了权重重置
...
class MergedLinear(nn.Linear, LoraLayer):
    ...
    def train(self, mode: bool = True): # 还是不知道这里是干嘛的，一会儿要再留意一下
        nn.Linear.train(self, mode)
        self.lora_A.train(mode)
        self.lora_B.train(mode)
        if not mode and self.merge_weights and not self.merged:
